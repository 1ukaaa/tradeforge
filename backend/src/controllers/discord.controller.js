...
const pickEntryImage = (entry) => {
  ...
  const candidate = images.find((image) => sanitizeImageUrl(image?.src));
  return candidate ? sanitizeImageUrl(candidate.src) : null;
};
...
const publishPayload = async (req, res) => {
  ...
  try {
    ...
    console.log("[Discord] Message envoyé", {
    ...
    const reason = error?.response?.data?.message || error.message || "Échec de l'envoi Discord.";
    console.error("Discord publish error:", error?.response?.data || error.message);
