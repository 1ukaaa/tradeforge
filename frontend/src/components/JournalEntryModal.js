...
const SimpleMarkdownDisplay = ({ content }) => {
  const theme = useTheme();

  if (!content) return <Typography color="text.secondary">Aucun contenu.</Typography>;
  ...
  return (
    <Stack spacing={1.5}>
      {lines.map((line, index) => {
        ...
        if (!trimmed) return <Box key={index} sx={{ height: 8 }} />; // Spacer for empty lines

        // 1. Detect Headers (### or 1. **)
        const isHeader = trimmed.startsWith('###') || /^\d+\.\s\*\*/.test(trimmed);
        if (isHeader) {
          const cleanHeader = trimmed.replace(/^###\s*/, '').replace(/^\d+\.\s\*\*/, '').replace(/\*\*$/, '').replace(/:$/, '');
          return (
            <Box key={index} sx={{ mt: 2, mb: 1 }}>
              ...
              <Box sx={{ height: 2, width: 40, bgcolor: 'primary.main', mt: 0.5, borderRadius: 1, opacity: 0.3 }} />
            </Box>
          );
        }

        // 2. Detect List Items (* or -)
        const isList = trimmed.startsWith('* ') || trimmed.startsWith('- ') || trimmed.startsWith('• ');
        if (isList) {
          const cleanList = trimmed.replace(/^[\*\-\•]\s*/, '');
          return (
            <Stack key={index} direction="row" spacing={1.5} sx={{ pl: 2 }}>
              <Box sx={{ minWidth: 4, height: 4, borderRadius: '50%', bgcolor: 'text.secondary', mt: 1.2 }} />
        ...
        // 3. Normal Text
        return (
          <Typography key={index} variant="body1" sx={{ lineHeight: 1.7, color: 'text.secondary' }}>
            {parseBold(trimmed)}
          </Typography>
        );
      })}
          ...
          <DialogContent sx={{ p: 0, height: '100%' }}>
            <Grid container sx={{ height: '100%' }}>
              ...
              <Grid item xs={12} md={8} sx={{ p: 4, overflowY: 'auto', borderRight: { md: `1px solid ${theme.palette.divider}` } }}>
                ...
                {/* The Content - CUSTOM MARKDOWN RENDERER */}
                <SimpleMarkdownDisplay content={displayEntry.content} />
